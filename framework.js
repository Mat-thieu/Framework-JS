...
var Framework = {
    ...
    analyzeHash : function(hash){
        ...
        if(hash == '#/' || hash == '#' || hash == ''){
            ...
            var countSlashes = function(val){ return (val.match(/\//g)||[]).length; }
            var matchRoute = function(namespace){
                ...
                frameworkCache.namespaceRoutes[namespace].some(function(routeVal, routeInd){
                    ...
                    if(match){
                        var thisRouteSlashes = countSlashes(route);
                        var thisMatchSlashes = countSlashes(hash);

                        // Check if it's an exact match
                        if(thisRouteSlashes == thisMatchSlashes){
                            var params = {};
                            // Handle a case with and without parameters
                            foundMatch = true;
                            if(routeVal['params'].length !== 0){
    ...
    init : function(settings){
        ...
        // Fire analyzeHash() for the first time
...
Namespace.prototype.listen = function(routeName, func){
...
var _get = {
    template : function(name, data, cb){
        if(name in frameworkCache.templates){
            ...
            logDebug('ajax', 'Retrieving template "'+name+'" from server');
        ...
        if(!Array.isArray(templates)){
            ...
            templates.forEach(function(name, ind){
                ...
                if(name in frameworkCache.templates){
                    ...
                    loadState.loaded++;
                    logDebug('ajax', 'Retrieving template "'+name+'" from cache');
...
var _view = {
    set : function(){
        var view = frameworkSettings.viewOutput;
        view.innerHTML = '';
        if(arguments[0] !== '') for (var i = 0; i < arguments.length; i++) view.appendChild(arguments[i]);
    }
}
